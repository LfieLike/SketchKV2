...
class KVquant_unit:
    def __init__(self):
        len = 16050
        self.quantized_tensor_KV = torch.empty((1, 32, len, 128),device='cpu', dtype=torch.uint8 ,pin_memory=True)
        self.device = 'cpu'
        self.bit_num = 4
        self.head_dim = 128
        self.quant_param = torch.empty((1, 32, len, 4),device='cpu', dtype=torch.float,pin_memory=True)
        self.first = True
...
class SkectchCache(Cache):
    ...
    def __init__(self,
                 layernum=32
                 ,buff_shape=(1,32,16050,128),dtype=torch.float16,
                 device='cpu') -> None:
        super().__init__()
...
class pruneCache(Cache):
    ...
    def update(
        ...
        key_sign = torch.sign(self.key_cache[layer_idx])
        score = torch.matmul(query_states[:,:,-1:,:], key_sign.half())
        hlaf_len = int(key_states.shape[-2]*self.imp_rate)
        _,sort_index = torch.topk(score[...,:-self.window_size],k=hlaf_len,dim = -1,sorted=False)
        sort_index = sort_index.squeeze(dim=-2)
        indices_expanded = sort_index.unsqueeze(-1).expand(-1, -1, -1, self.head_dim)
        value_states = torch.gather(self.value_cache[layer_idx], 2, indices_expanded).contiguous()
        key_states = torch.gather(self.key_cache[layer_idx], 2, indices_expanded).contiguous()
