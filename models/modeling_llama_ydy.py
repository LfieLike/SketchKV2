...
# DENSE_LAYER=[0] # llama3
DENSE_LAYER=[0,1] # llama2
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        partial_att = torch.matmul(query_states[:,:,-1:,:], key_states.transpose(2, 3)) / math.sqrt(self.head_dim)
        topk_values, topk_indices = torch.topk(partial_att, k=int(128), dim=-1, sorted=True)
        # print("layer", self.layer_idx, "topk_indices",torch.sort(topk_indices,dim=-1))
        if q_len != 1:
            # prefill
            ...
            selected_key_states = torch.gather(key_states, dim=-2, index=topk_indices.squeeze(-2).unsqueeze(-1).expand(-1, -1, -1, key_states.size(-1)))
            selected_value_states = torch.gather(value_states, dim=-2, index=topk_indices.squeeze(-2).unsqueeze(-1).expand(-1, -1, -1, value_states.size(-1)))
            # torch.set_printoptions(threshold=torch.inf)
            # print(self.layer_idx, channel_top_indices[0][0])


            attn_weights = torch.matmul(query_states, selected_key_states.transpose(2, 3)) / math.sqrt(self.head_dim)
            attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32).to(query_states.dtype)
            attn_weights = nn.functional.dropout(attn_weights, p=self.attention_dropout, training=self.training)
