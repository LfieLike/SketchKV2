...
if __name__ == '__main__':
    ...
    if 'llama' in model_args.model_name_or_path.lower() or 'longchat' in model_args.model_name_or_path.lower():
        if model_args.method == "kivi":
            from models.modeling_llama_ydy import LlamaForCausalLM
            ...
            from models.modeling_llama_1103 import LlamaForCausalLM
            model = LlamaForCausalLM.from_pretrained(
                pretrained_model_name_or_path=model_args.model_name_or_path,
                config=config,
                cache_dir=training_args.cache_dir,
    ...
    for dataset in datasets:
        if data_args.e:
            ...
            if not os.path.exists(f"pred/{model_name}_ydy"):
                os.makedirs(f"pred/{model_name}_ydy")
            out_path = f"pred/{model_name}_ydy/{dataset}.jsonl"
