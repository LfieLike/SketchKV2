...
// batch_id和head_id不能*NUM_PER_THREAD*WARPS_PER_BLOCK 来表示位移，因为可能会有越界的线程。
    ...
    __shared__ uint8_t share_outerlier_quant[12*8];
    __shared__ uint8_t share_outerlier_idx[12*8];
    __shared__ half share_outerlier_zp[12*8];
    __shared__ half share_outerlier_scale[12*8];
    if(pro_id * NUM_PER_THREAD*WARPS_PER_BLOCK + vec_id*128>=len*EMB_DIM){
        return;
    }
    // 先写死，每个线程向量化的读16个uint8，8个half
    if(th_id<6){
        reinterpret_cast<int4*>(share_outerlier_quant)[th_id] = reinterpret_cast<int4*>(key_outlier_quant+base_index/128*outlier_num)[th_id];
    }
    if()
    if(th_id<12){
        reinterpret_cast<int4*>(share_outerlier_zp)[th_id] = reinterpret_cast<int4*>(quant_outlier_zp+base_index/128*outlier_num)[th_id];
        reinterpret_cast<int4*>(share_outerlier_scale)[th_id] = reinterpret_cast<int4*>(quant_outlier_scale+base_index/128*outlier_num)[th_id];
    }
    
    // 获取当前线程计算的偏移量，每个线程计算NUM_PER_THREAD个元素，每thread_num_per_emb个线程计算计算一个
    // 每个线程处理NUM_PER_THREAD个元素，也就是每个线程读取一个uint8_t
    int dequant_shfit = base_index + th_id*8;
    uint8_t onebit_key = compressed_key[dequant_shfit/8];
