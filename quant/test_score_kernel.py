...
def simple_test(query_states,org_key):
    return torch.matmul(query_states.to(torch.float32).unsqueeze(-2),org_key.to(torch.float32).transpose(-1,-2)).to(torch.float16)
...
def test_my_score_cuda():
    ...
    for i in range(4):
        ...
        true_result = simple_test(query_states=query_states, org_key=res)
    print("-------------")
    # 打印结果
    print("diff:",(true_result.to(torch.float32).view(-1)-result.to(torch.float32).view(-1)).abs().sum())
