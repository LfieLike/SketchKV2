...
class train_attn(nn.Module):
    def __init__(self,mask,k_dim,v_dim,num_heads,lowrank_dim):
        super().__init__()
    ...
    def forward(self,query_states,key_states,value_states):
        ...
        # print("1111,",key_states.shape)
        # mask = 
        cos, sin = self.rotary_emb(value_states, seq_len=q_len)
        tensor = torch.arange(0, q_len)
        # 调整tensor的形状为[1, 2048]
        self.pos = tensor.view(1, q_len)
        query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, self.pos)
        cnt = key_states
        org_val = self.cal_attn(query_states, key_states,value_states)
        # key_states = quant_mock(key_states)
        # key_states = key_states.transpose(1, 2).view(bsz*q_len, num_heads, head_dim)
        ...
        lowrank_val = self.cal_attn(query_states, key_states,value_states)
        return lowrank_val,org_val
bsz = 1 
num_heads = 32
head_dim = 128

mask = torch.load("/root/KIVI-lmeval/data/attention_mask.pt")
model = train_attn(mask=mask, k_dim=head_dim, v_dim=head_dim,num_heads=32, lowrank_dim=128).to('cuda:1')
...
def test(epoch,model):
    model.eval()
    devices = "cuda:1"
    sufixpath = "/root/KIVI-lmeval/data/"
    a = random.randint(0,100)
    # query_states = torch.load(sufixpath+a+"_20query_states.pt").float()
    # key_states = torch.load(sufixpath+a+"_20key_states.pt").float()
    # value_states = torch.load(sufixpath+a+"_20value_states.pt").float()
    query_states = query_list[a].float()
    key_states = key_list[a].float()
    value_states = value_lsit[a].float()
    # print(value_lsit[a].dtype)
    q_len = key_states.shape[1]
    # print(key_states.shape)
    query_states = query_states.view(bsz, q_len, num_heads, head_dim).transpose(1, 2)
    key_states = key_states.view(bsz, q_len, num_heads, head_dim).transpose(1, 2)
    value_states = value_states.view(bsz, q_len, num_heads, head_dim).transpose(1, 2)
    # query_states, key_states, value_states = query_states.to(devices), key_states.to(devices), value_states.to(devices)
    output, labels = model(query_states.to(devices), key_states.to(devices), value_states.to(devices))
    if epoch%20 == 0:
        draw_matrix(output[0][2],labels[0][2])
    loss = loss_function(output, labels)
    return loss.item()
    # print(f"eval Loss: {loss.item()}")
    

for i in range(1901,2040):
    sufixpath = "/root/KIVI-lmeval/data/"
    a=  str(i)
    query_states = torch.load(sufixpath+a+"_20query_states.pt").cpu()
    key_states = torch.load(sufixpath+a+"_20key_states.pt").cpu()
    value_states = torch.load(sufixpath+a+"_20value_states.pt").cpu()
    query_list.append(query_states)
    key_list.append(key_states)
    value_lsit.append(value_states)
for epoch in range(10000):  # Number of epochs
    # batch_size = 32
    model.train()
    devices = "cuda:1"
    a = str(random.randint(0,1900))
    # query_states = torch.cat(random.sample(query_list, batch_size)
    # query_states = query_list[a].float()
    # key_states = key_list[a].float()
    # value_states = value_lsit[a].float()
    sufixpath = "/root/KIVI-lmeval/data/"
...
torch.save(model.state_dict(), '20layer.pth')  
    # print("cnt")